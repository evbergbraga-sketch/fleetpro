...
function _renderCondutores(){
  ...
  wrap.innerHTML = _condutoresLista.map((c,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--bg2);border:1px solid var(--border2);border-radius:8px;padding:8px 10px;margin-bottom:6px">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:500">${c.nome}</div>
        <div style="font-size:11px;color:var(--muted)">CPF: ${c.cpf||'não informado'}</div>
      </div>
...
function _adicionarCondutor(){
  ...
  _condutoresLista.push({nome, cpf});
  document.getElementById('novo-condutor-nome').value = '';
  document.getElementById('novo-condutor-cpf').value = '';
  _renderCondutores();
}
...
async function _carregarCondutoresCliente(){
  ...
  if(data?.length){
    _condutoresLista = data.map(d=>({nome:d.nome, cpf:d.cpf||'', id:d.id}));
  }
  _renderCondutores();
}
...
async function converterReservaContrato(id){
  ...
  goPage('contratos');
  setTimeout(()=>{
    const selCli=document.getElementById('c-cli'); if(selCli){ selCli.value=r.cliente_id; _carregarCondutoresCliente(); }
    ...
    if(selVei){
      ...
      selVei.value=r.veiculo_id; autoFillContrato();
    }
    const elIni=document.getElementById('c-ini'); if(elIni) elIni.value=r.data_inicio?.slice(0,16)||'';
    const elFim=document.getElementById('c-fim'); if(elFim) elFim.value=r.data_fim?.slice(0,16)||'';
    ...
    window._reservaOrigemId=id; window._reservaValorPago=r.valor_pago||0;
    if(r.valor_pago>0) notify(`Reserva com R$ ${Number(r.valor_pago).toFixed(2).replace('.',',')} já pago — será abatido.`,'success');
    previewContrato();
  },300);
}
