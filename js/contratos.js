...
function renderCal(){
  ...
  for(let d=1;d<=days;d++){
    ...
    html+=`<div class="cal-day ${isT?'today':''}" id="cal-d-${d}" onclick="calSelectDay(${d})"><span>${d}</span>${dots?`<div class="dots">${dots}</div>`:''}</div>`;
  }
    ...
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px;background:var(--bg3);border-radius:8px;border:1px solid var(--border)">
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <div>
          ...
          <div style="font-size:11px;color:var(--muted)">${v.placa}</div>
          ${extra}
        </div>
      </div>
